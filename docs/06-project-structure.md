...
The FastAPI application that implements the **Service Layer** defined in the [Concurrency](./02-concurrency.md) document — the system's HTTP boundary. It handles request validation, response serialization, and the delegation of heavy work to background workers across the message broker.
...
└── src/server/
    ...
    └── modules/             # Route handlers grouped by domain resource.
        ...
        ├── task/            # Task status polling endpoint (queries the result
        │                    # backend).
        ├── video/           # Video ingestion, retrieval, download, and
        │   │                # deletion.
        ...
        ├── estimation/      # Estimation submission, retrieval, download, and
        │   │                # deletion.
        ...
        └── visualization/   # Visualization submission, retrieval, download,
            │                # and deletion.
            ├── submit/      # POST endpoint for visualization task submission.
            ├── get/         # GET endpoint for visualization metadata
            │                # retrieval.
            ├── download/    # GET endpoint for rendered MP4 download.
            └── delete/      # DELETE endpoint for visualization removal.
```

Route handlers are organized by domain resource under `modules/`, with each resource directory containing subdirectories for each supported HTTP operation. This structure makes the API surface navigable by resource and operation rather than by technical artifact type, aligning with the resource-oriented API decisions in the [Conceptual Overview](./01-conceptual-overview.md).

The `exception_handlers/` subdirectory implements the unified error-handling strategy, translating internal exceptions into generic, decoupled error responses as defined in [Request Lifecycles](./04-request-lifecycles.md).

### 2. `packages/worker-inference/`

The GPU-bound worker process that executes the **pose estimation** tasks defined in the [Concurrency](./02-concurrency.md) document — the system's isolated inference context. It consumes tasks from the message broker, runs the `ensam3d_inference` pipeline on the GPU, and persists the resulting artifacts.

```bash
worker-inference/
├── pyproject.toml
└── src/worker_inference/
    ├── config.py                      # Worker-specific configuration (model
    │                                  # paths, batch sizes).
    ├── main.py                        # Celery worker entry point with GPU
    │                                  # initialization.
    ├── _state.py                      # Global pipeline instance management
    │                                  # (loaded once at startup).
    └── tasks/
        └── estimate/                  # Task implementation for 3D pose
            │                          # estimation.
            ├── metadata_persistor.py  # Saves estimation metadata to
            │                          # PostgreSQL.
...
The CPU-bound worker process that executes the **annotation rendering** tasks defined in the [Concurrency](./02-concurrency.md) document — the system's isolated post-processing context, free of any ML or GPU dependencies. It consumes tasks from the message broker, renders pose overlays onto the source video, and persists the encoded result.

```bash
worker-default/
├── pyproject.toml
└── src/worker_default/
    ├── config.py                # Worker-specific configuration (FFmpeg
    │                            # settings).
...
This worker uses a `prefork` pool with `concurrency=4`, letting multiple CPU-bound rendering tasks run in parallel — and scale independently of the GPU-bound inference pool (**NFR‑6**). Its complete isolation from the inference worker keeps GPU memory dedicated to inference, never pinned by a process that performs no ML work (**NFR‑5**).

### 4. `packages/shared/`

Centralizes all infrastructure client code and shared utilities consumed by the [Service Layer](./06-project-structure.md) and both worker types, implementing the concrete clients for the architectural roles defined in the [Concurrency](./02-concurrency.md) and [Domain Model](./03-domain-model.md) documents — the message broker and result backend, the SQL database, and object storage.

```bash
shared/
├── pyproject.toml
└── src/
    ├── base_lib/                   # Foundational utilities shared across all
    │   │                           # packages.
    │   ├── base_config.py          # Base configuration class with environment
    │   │                           # variable loading.
    │   └── logger.py               # Structured logging setup.
    │
    ├── postgres_lib/               # SQL Database layer.
    ...
    │
    ├── minio_lib/                  # Object storage layer.
    │   ├── config.py               # MinIO connection settings.
    │   ├── operations.py           # Upload, download, presigned URL
    │   │                           # generation.
...
└── src/scripts/
    ...
    │   ├── shared.py                 # Common utilities (API URL construction,
    │   │                             # health checks).
...
The `quick_start/` scripts orchestrate infrastructure provisioning, database migration, and process launch in the correct dependency order, eliminating manual setup steps. The `utils/` scripts generate infrastructure initialization artifacts from environment variables, provisioning each database user and Redis ACL with only the privileges its consumer needs (**NFR‑11**).
...
└── src/benchmarks/
    ...
    │   ├── __init__.py            # Common functions (CPU/GPU detection,
    │   │                          # service management).
    ...
    └── parallel_pipeline.py       # Entry point for parallel pipeline
                                   # benchmark.
```
...
Alembic migration environment managing the evolution of the **SQL Database** schema. Contains the configuration file (`alembic.ini`), migration script template, and all versioned migration files. Migrations are generated via `just db-revision` (creates a new migration script from model changes) and applied via `just db-migrate` (executes pending migrations against the database).
...
The two Compose stacks realize the dual-mode requirement of **NFR‑7**: the infrastructure dependencies always run in containers, while application code either runs on the host for fast iteration or is containerized alongside them for deployment — the same codebase in both, with no manual rework to switch between modes. The two modes therefore differ in exactly one dimension — where application code runs:

| Mode | Compose file | Infrastructure (PostgreSQL, Redis, MinIO) | Application code (Service Layer, workers) | Purpose |
|------|--------------|-------------------------------------------|-------------------------------------------|---------|
| **Local development** | `compose.local.yml` | Containerized | Runs directly on the host | Rapid iteration — edit and restart application code without rebuilding images. |
| **Production deployment** | `compose.deploy.yml` | Containerized | Containerized | Fully isolated, reproducible stack with every service in its own container. |

The `postgres/`, `redis/`, and `minio/` subdirectories contain initialization artifacts generated by the infrastructure scripts in `packages/scripts/utils/` (these generated files are git-ignored by default, so they will not appear in the repository — they are produced locally on first setup).

### 9. `config/`

Environment variable templates for local development and production deployment modes. By separating configuration from code, the same codebase runs in either mode purely by supplying a different `.env` — no source changes when switching environments (**NFR‑7**).