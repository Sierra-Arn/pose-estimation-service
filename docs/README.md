...
flowchart TB
    ...
    subgraph System["Human Pose Estimation Service"]
        ...
        subgraph Workers["Background Workers"]
            InferenceWorker["Inference Worker<br/><i>Celery + PyTorch + CUDA</i><br/>GPU-accelerated<br/>3D pose estimation"]
            PostProcessingWorker["Post-processing Worker<br/><i>Celery + FFmpeg</i><br/>CPU-bound video<br/>rendering & encoding"]
        end

        subgraph Storage["Data Storage"]
            PostgreSQL["PostgreSQL<br/><i>Relational Database</i><br/>Metadata Store:<br/>Video, Estimation,<br/>Visualization records"]
            Redis["Redis<br/><i>In-memory Store + Broker</i><br/>State Store (job status)<br/>+ Task Queue (Celery broker)"]
    ...
    Redis -->|"Dispatch inference<br/>tasks"| InferenceWorker
    Redis -->|"Dispatch rendering<br/>tasks"| PostProcessingWorker
...
- To **install, provision infrastructure, and deploy the system** in local or fully containerized modes, see
    [10-deployment-guide.md](10-deployment-guide.md).